    ...
    if isinstance(diskio, list):
        ...
        if any(isinstance(v, dict) for v in vals):
            ...
            for k, v in diskio.items():
                if k.endswith(".read_bytes") or k.endswith(".read_bytes_rate_per_sec"):
                    read += v
                if k.endswith(".write_bytes") or k.endswith(".write_bytes_rate_per_sec"):
                    write += v
    read /= _MB
    write /= _MB
    # GPU utilization
...
def start_glances(tmp_json: Path) -> subprocess.Popen:
    ...
    p = subprocess.Popen(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, text=True, preexec_fn=os.setsid)
    time.sleep(1.5)
    ...
    try:
        ...
        for line in text.splitlines():
            try:
                ...
                logging.error("Error parsing JSON line: %s", line)
                logging.exception(e)
        return out
...
def load_static(logfile: Path, history: Deque[dict], out_dir: Path, plot: bool):
    ...
    if plot:
        ...
        fig, axes = new_fig(len(PANELS))
        redraw(fig, axes, df)
        fig.savefig(out_dir / "static.png")
        plt.show()
...
def main():
    ...
    ap.add_argument("-o", "--out-dir", default="outputs", type=Path, help="Directory for logs, glances JSON, and plots")
