...
def extract(j: Dict[str, Any]) -> Dict[str, float]:
    ...
    if isinstance(diskio, list):
        ...
        if any(isinstance(v, dict) for v in vals):
            ...
            # flat mapping: keys like 'disk0.read_bytes'
    ...
    if isinstance(gpu_src, list) and gpu_src:
        gpu0 = gpu_src[0]
    elif isinstance(gpu_src, dict) and gpu_src:
        gpu0 = next(iter(gpu_src.values()))
    else:
        gpu0 = {}
    ...
    for r in recs:
        ...
        if last_ts is None or ts>last_ts:
            if "cpu_pct" in r and "disk_read_mb" in r:
                ...
                row={"ts":ts}|extract(r)
            history.append(row); last_ts=ts

# ───────────────────────── Main ────────────────────────────────────
def main():
    ap = argparse.ArgumentParser(description="Simplified metrics tracing & logging")
    ap.add_argument("-f","--file", default="outputs/metrics.log.ndjson",
                    help="ND-JSON log (live or replay)")
    ap.add_argument("--live", action="store_true", help="Run live logging")
    ap.add_argument("--plot", action="store_true", help="Enable plotting in live mode")
