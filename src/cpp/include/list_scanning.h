...
inline void batched_scan_list(const float *query_vecs,
    ...
    if (alloc) {
        ...
        indices = (int64_t *) malloc(num_queries * k_max * sizeof(int64_t));
    }


    if (metric == faiss::METRIC_INNER_PRODUCT) {
        faiss::float_minheap_array_t res = {size_t(num_queries), size_t(k_max), indices, distances};
        faiss::knn_inner_product(query_vecs, list_vecs, dim, num_queries, list_size, &res, nullptr);
    } else if (metric == faiss::METRIC_L2) {
        faiss::float_maxheap_array_t res = {size_t(num_queries), size_t(k_max), indices, distances};
    ...
    if (list_ids != nullptr) {
        for (int i = 0; i < num_queries; i++) {
            for (int j = 0; j < k_max; j++) {
                indices[i * k_max + j] = list_ids[indices[i * k_max + j]];
            }
        }
    }
    ...
    for (int i = 0; i < num_queries; i++) {
        topk_buffers[i]->batch_add(distances + i * k_max, indices + i * k_max, k_max);
    }
